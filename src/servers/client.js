...
fs.readdir("./src/servers/commands/", (err, files) => {
  ...
  console.log(`[botcity.xyz/servers]: ${files.length} comando cargado.`);
...
serverClient.on('ready',async () => {
    console.log("[botcity.xyz/servers]: Bot conectado correctamente como "+serverClient.user.tag+".");
    let serversdata = require("../../src/database/models/servers/server.js");
    const servers = await serversdata.find();
    serverClient.user.setPresence({ activity: { type: 'WATCHING', name: + servers.length+' Servers | !help' }, status: "dnd" });
});
